"""Conditional remote prompt resolution with immutable, last-known-good snapshots."""
...
class RemotePromptError(RuntimeError):
    """A remote prompt could not be loaded or validated."""
...
class RemotePromptStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._remote_active = bool(enabled and self._url and self._bindings)
        self._client = client
    ...
    @property
    def remote_keys(self) -> frozenset[str]:
        """Keys requiring remote prompts instead of local fallbacks."""
        return frozenset(self._bindings) if self._remote_active else frozenset()
    ...
    async def refresh_once(self, *, initial: bool = False) -> None:
        ...
        if not self._remote_active:
            return
        errors: list[RemotePromptError] = []
        for binding in self._bindings.values():
            try:
                await self._refresh_binding(binding)
            except RemotePromptError as exc:
                self._last_errors[binding.key] = str(exc)
                if initial:
                    errors.append(exc)
                else:
                    logger.warning(
                        "Remote prompt refresh failed for key=%s name=%s; retaining the last-known-good version: %s",
                        binding.key,
                        binding.name,
                        exc,
                    )
        if errors:
            raise RemotePromptError(
                f"Initial remote prompt load failed for {len(errors)} binding(s)"
            ) from errors[0]
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            "mode": "remote" if self._remote_active else "embedded",
            "ready": not self._remote_active or len(self._snapshots) == len(self._bindings),
    ...
    def _validate_remote_configuration(self) -> None:
        ...
        if parsed.scheme not in {"http", "https"} or not parsed.netloc:
            raise RemotePromptError("Prompt resolve URL must be an HTTP(S) URL")
        ...
        if not self._auth_token:
            raise RemotePromptError("Prompt read credential is required")
        if self._refresh_interval_seconds <= 0:
            raise RemotePromptError("Remote prompt refresh interval must be positive")

    async def _refresh_binding(self, binding: PromptBinding) -> None:
        if self._client is None:
            raise RemotePromptError("Remote prompt HTTP client is not initialized")
        ...
        try:
            ...
            raise RemotePromptError("OpenObserve could not be reached") from exc

        if response.status_code == 304:
            if current is None:
                raise RemotePromptError("OpenObserve returned 304 without a cached version")
            self._mark_success(binding.key)
            return
        if response.status_code in {401, 403}:
            raise RemotePromptError("OpenObserve denied prompt read access")
        if response.status_code == 404:
            raise RemotePromptError("Configured prompt or label was not found")
        if response.status_code != 200:
            raise RemotePromptError(f"OpenObserve returned HTTP {response.status_code}")

        etag = response.headers.get("etag", "").strip()
        if not etag:
            raise RemotePromptError("OpenObserve prompt response did not include an ETag")
        try:
            snapshot = _parse_resolved(response.json(), binding, etag)
        except ValueError as exc:
            raise RemotePromptError("OpenObserve returned an invalid prompt response") from exc

        self._snapshots = {**self._snapshots, binding.key: snapshot}
        self._mark_success(binding.key)
        logger.info(
            "Loaded remote prompt key=%s name=%s version=%d label=%s",
            binding.key,
            snapshot.name,
            snapshot.version,
            snapshot.label,
