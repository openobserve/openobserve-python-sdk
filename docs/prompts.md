...
`PromptStore` caches text prompts from OpenObserve's prompt registry in memory. It can be used independently of the telemetry SDK and does not depend on an agent framework.

```python
import asyncio

from openobserve import PromptBinding, PromptStore
